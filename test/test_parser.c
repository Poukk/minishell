...
Test(parser_tests, test_simple_command) {
	...
	tokens = tokenize(&gc, "ls -l");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_simple_pipe) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "ls | grep txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_multiple_pipes) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "cat file | grep pattern | wc -l");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_pipes_with_redirections) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "ls | echo hello > file.txt | cat file.txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_input_redirection) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "cat < input.txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_output_redirection) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "echo hello > output.txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_both_redirections) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "cat < input.txt > output.txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_redirections_mixed_with_args) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "grep pattern < input.txt > output.txt");
	ast = parse(&gc, tokens, env);
...
Test(parser_tests, test_multiple_output_redirections) {
	...
	gc_init(&gc);
	env = create_test_env(&gc);
	tokens = tokenize(&gc, "echo hello > file1.txt > file2.txt > file3.txt");
	ast = parse(&gc, tokens, env);
