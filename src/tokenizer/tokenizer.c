		...
		t_tokenizer_status *status)
{
	if (is_separator_metachar(**current))
		...
		return (create_word_token(gc, current, status));
}
...
t_token	*tokenize(t_gc *gc, const char *input, t_tokenizer_status *status)
{
	t_token		*head;
	t_token		*token;
	const char	*current;
	...
	while (*current)
	{
		...
		if (!*current)
			break ;
		token = process_input_token(gc, &current, status);
		if (!token)
			return (NULL);
